...
class GetXMLTask3 : AsyncTask<String, Void, Document>() {
    ...
    override fun doInBackground(vararg urls: String): Document? {
        ...
        try {
            url =
                    URL("http://api.visitkorea.or.kr/openapi/service/rest/KorService/searchKeyword?ServiceKey=b3W0ZANT1VPYvbjqez0COkQwtfGXJBTyD0SoDy5EdkJUcldZXd5k9TM0qmNIcCdp21wQOA%2BE%2FzttN4hYU7ZvAA%3D%3D&keyword=${keyword}&areaCode=${areaCode}&sigunguCode=&cat1=&cat2=&cat3=&listYN=Y&MobileOS=ETC&MobileApp=TourAPI3.0_Guide&arrange=A&numOfRows=30&pageNo=1")
    ...
    override fun onPostExecute(doc: Document) {
        ...
        if(dataList.size == 0){
            (mContext as KeyWordActivity).noKeyWordSearchMethod()
        }

        var myAdapter: MyAdapter = MyAdapter(dataList)
