...
class GetXMLTask2 : AsyncTask<String, Void, Document>() {
    ...
    override fun doInBackground(vararg urls: String): Document? {
        ...
        try {
            url =
                    URL("http://api.visitkorea.or.kr/openapi/service/rest/KorService/locationBasedList?ServiceKey=b3W0ZANT1VPYvbjqez0COkQwtfGXJBTyD0SoDy5EdkJUcldZXd5k9TM0qmNIcCdp21wQOA%2BE%2FzttN4hYU7ZvAA%3D%3D&contentTypeId=${contenttypeid}&mapX=${MyLocationLongitude}&mapY=${MyLocationLatitude}&radius=${dist}&listYN=Y&MobileOS=ETC&MobileApp=TourAPI3.0_Guide&arrange=A&numOfRows=30&pageNo=1")
    ...
    override fun onPostExecute(doc: Document) {
        ...
        for (i in 0 until nodeList.length) {
            ...
            var dist = fstElmnt.getElementsByTagName("dist")
            if (dist?.item(0)?.childNodes?.item(0)?.nodeValue != null) {
                data.location_dist = dist.item(0).childNodes.item(0).nodeValue.toDouble()
            }

            if (!data.location_contenttype.equals("25")) {
                dataList.add(data)
            }
        }

        (mContext as NearLocationActivity).locationMarker()

        if(dataList.size == 0){
            (mContext as NearLocationActivity).noNearLocation()
        }

        var myAdapter2: MyAdapter2 = MyAdapter2(dataList)
